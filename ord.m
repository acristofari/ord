        ...
        for i = 1:length(opts_field)
            switch char(opts_field(i))
                ...
                case 'n_initial_atoms'
                    ...
                    if (~isscalar(n_atoms_in) || ~isnumeric(n_atoms_in) || ~isreal(n_atoms_in) || n_atoms_in<1 || n_atoms_in>n_atoms)
                       error(['In the options, ''n_initial_atoms'' must be between 1 and ' num2str(n_atoms) '.']);
                    end
    ...
    while (true)
        ...
        if (n_atoms_in > 1)
            ...
            if (max_n_f_local > 0)
                ...
                [x,f,df_simplex_info,sampling] = df_simplex(obj,A_k,y,df_simplex_opts);
                y = df_simplex_info.y;
        ...
        if (n_f>=max_n_f ||it >= max_it || (v_added && f_next<=min_f))
            if (v_added)
